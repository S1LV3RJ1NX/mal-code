...
from utils import decode_text, encode_text, generate
...
def load_weights(gpt, gpt_hf):
    ...
    gpt.positional_encoding.weight = assign_check(
        gpt.positional_encoding.weight, d["wpe.weight"]
    )
    ...
    for b in range(GPT_CONFIG_124M["n_layers"]):
        ...
        gpt.transformer_blocks[b].mha.W_query.weight = assign_check(
            gpt.transformer_blocks[b].mha.W_query.weight, q_w.T
        )
        gpt.transformer_blocks[b].mha.W_key.weight = assign_check(
            gpt.transformer_blocks[b].mha.W_key.weight, k_w.T
        )
        gpt.transformer_blocks[b].mha.W_value.weight = assign_check(
            gpt.transformer_blocks[b].mha.W_value.weight, v_w.T
        )

        # Split and assign Q,K,V bias terms
        q_b, k_b, v_b = np.split(d[f"h.{b}.attn.c_attn.bias"], 3, axis=-1)
        gpt.transformer_blocks[b].mha.W_query.bias = assign_check(
            gpt.transformer_blocks[b].mha.W_query.bias, q_b
        )
        gpt.transformer_blocks[b].mha.W_key.bias = assign_check(
            gpt.transformer_blocks[b].mha.W_key.bias, k_b
        )
        gpt.transformer_blocks[b].mha.W_value.bias = assign_check(
            gpt.transformer_blocks[b].mha.W_value.bias, v_b
        )

        # Load attention output projection
        gpt.transformer_blocks[b].mha.out_proj.weight = assign_check(
            gpt.transformer_blocks[b].mha.out_proj.weight,
            d[f"h.{b}.attn.c_proj.weight"].T,
        )
        gpt.transformer_blocks[b].mha.out_proj.bias = assign_check(
            gpt.transformer_blocks[b].mha.out_proj.bias, d[f"h.{b}.attn.c_proj.bias"]
        )

        # Load feed-forward network weights
        # First linear layer (expansion)
        gpt.transformer_blocks[b].ffn.layers[0].weight = assign_check(
            gpt.transformer_blocks[b].ffn.layers[0].weight,
            d[f"h.{b}.mlp.c_fc.weight"].T,
        )
        gpt.transformer_blocks[b].ffn.layers[0].bias = assign_check(
            gpt.transformer_blocks[b].ffn.layers[0].bias, d[f"h.{b}.mlp.c_fc.bias"]
        )
        # Second linear layer (projection)
        gpt.transformer_blocks[b].ffn.layers[2].weight = assign_check(
            gpt.transformer_blocks[b].ffn.layers[2].weight,
            d[f"h.{b}.mlp.c_proj.weight"].T,
        )
        gpt.transformer_blocks[b].ffn.layers[2].bias = assign_check(
            gpt.transformer_blocks[b].ffn.layers[2].bias, d[f"h.{b}.mlp.c_proj.bias"]
        )

        # Load layer normalization weights
        gpt.transformer_blocks[b].layer_norm1.weight = assign_check(
            gpt.transformer_blocks[b].layer_norm1.weight, d[f"h.{b}.ln_1.weight"]
        )
        gpt.transformer_blocks[b].layer_norm1.bias = assign_check(
            gpt.transformer_blocks[b].layer_norm1.bias, d[f"h.{b}.ln_1.bias"]
        )
        gpt.transformer_blocks[b].layer_norm2.weight = assign_check(
            gpt.transformer_blocks[b].layer_norm2.weight, d[f"h.{b}.ln_2.weight"]
        )
        gpt.transformer_blocks[b].layer_norm2.bias = assign_check(
            gpt.transformer_blocks[b].layer_norm2.bias, d[f"h.{b}.ln_2.bias"]
        )
    ...
    gpt.linear_projection.weight = assign_check(
        gpt.linear_projection.weight, d["wte.weight"]
    )
...
gpt_hf = GPT2Model.from_pretrained(
    "openai-community/gpt2", cache_dir="pretrained-checkpoints"
)
gpt_hf.eval()
