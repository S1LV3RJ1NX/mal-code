# Chapter 3: The Llama3 Herd of Models

_Note:_ Make sure to follow the instructions in the [README](../README.md) to install the dependencies.

## Overview

In this chapter, we will implement the `LLama3.2 3B` model and use it's pretrained weights to generate the text.

## Folder Structure

- `ch3/`
  - `components/` - Components of the llama3 model (rms_norm, GQA, feed-forward, etc.)
...
- Download the pre-trained weights in the `pretrained-weights` folder by executing:

```bash
python download_llama.py
```

> Note: You need to have a Hugging Face account and a token with access to the model. You can get the token from [here](https://huggingface.co/settings/tokens).
...
Generated Text:
--------------------------------------------------
Initial text:> being a founder means
Output text:
 being a founder means you have to be a jack of all trades. you have to be able to do everything. you have to be able to
--------------------------------------------------
```

## Testing individual components

- Certain components have code within `__main__` block to test the implementation.
...
- For example, to test the `GroupedQueryAttention` component, run the following command:

```bash
python -m components.grouped_query_attention
```
